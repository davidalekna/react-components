...
export const DataBrowserContext = React.createContext({
    columnFlex: [],
...
export class DataBrowser extends React.Component {
    constructor() {
        ...
        this.switchViewType = ({ type = DataBrowser.stateChangeTypes.switchView, viewType = '', } = {}) => {
            if (this.props.viewsAvailable.includes(viewType)) {
                this.internalSetState({ type, viewType }, () => this.props.onSwitchViewType(this.getState().viewType));
            }
            else {
    ...
    render() {
        ...
        return (React.createElement(DataBrowserContext.Provider, { value: this.state }, ui));
    }
}
...
DataBrowser.Consumer = DataBrowserContext.Consumer;
